...
struct State {
    ...
    State() {
        L = luaL_newstate();
        luaL_openlibs(L);
        cout << "created lua state\n";
    }
    ~State() {
        lua_close(L);
        cout << "destroyed lua state\n";
    }
};
...
void Library::Run() {
    const struct luaL_reg mylib[] = {{"add5", add5}, {"add10", add10}, {NULL, NULL}};
    ...
    if (status) { cerr << "error = " << lua_tostring(state_p->L, -1) << "\n"; }
}
...
void Complex::Run() {
    ints.push_back(10);
    const struct luaL_reg veclib[] = {{"size", vec_size}, {"at", vec_at}, {NULL, NULL}};
    cout << "opening library for vec...\n";

    cout << "lua_gettop = " << lua_gettop(state_p->L) << "\n";
    intpvec ipv =
        (intpvec)lua_newuserdata(state_p->L, sizeof(vector<int> *));  // first on stack
    ...
    *ipv = (&ints);                                 // set the
    {                                               // method 1
        ...
        lua_setfield(state_p->L, -2,
                     "__index");  // attaching __index to metatable and pop
